const fs = require('fs');
const file = 'src/components/Admin.tsx';
let content = fs.readFileSync(file, 'utf8');

// Add states
content = content.replace("const [osSearch, setOsSearch] = useState('');", 
`const [osSearch, setOsSearch] = useState('');
  const [osStatusFilter, setOsStatusFilter] = useState('');
  const [osDateFrom, setOsDateFrom] = useState('');
  const [osDateTo, setOsDateTo] = useState('');`);

// Update search filters
const filterLogic = `
  const filteredOS = orders.filter(os => {
    let match = true;
    if (osStatusFilter && os.status !== osStatusFilter) match = false;
    
    if (osDateFrom || osDateTo) {
      // os.createdAt is ISO string
      const osDate = new Date(os.createdAt);
      if (osDateFrom) {
         const from = new Date(osDateFrom + 'T00:00:00');
         if (osDate < from) match = false;
      }
      if (osDateTo) {
         const to = new Date(osDateTo + 'T23:59:59');
         if (osDate > to) match = false;
      }
    }
    
    if (osSearch) {
      const s = osSearch.toLowerCase();
      match = match && (
        (os.number && os.number.toLowerCase().includes(s)) ||
        (os.driverName && os.driverName.toLowerCase().includes(s)) ||
        (os.driverPlate && os.driverPlate.toLowerCase().includes(s)) ||
        (os.status && os.status.toLowerCase().includes(s)) ||
        (os.clientName && os.clientName.toLowerCase().includes(s))
      );
    }
    return match;
  });
`;

content = content.replace("const totalOrders = orders.length;", filterLogic + "\n  const totalOrders = orders.length;");

// Update table filter
content = content.replace(/\{orders\.filter\(os => \{[\s\S]*?\}\)\.length === 0 \? \(/g, 
`{filteredOS.length === 0 ? (`);

content = content.replace(/: orders\.filter\(os => \{[\s\S]*?\}\)\.map\(\(os\) => \(/g, 
`: filteredOS.map((os) => (`);

// Replace search input area to include all filters
const searchUI = `
                <div className="relative w-full md:w-64">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  <input 
                    type="text" 
                    placeholder="Buscar por OS, Motorista ou Status..."
                    value={osSearch}
                    onChange={(e) => setOsSearch(e.target.value)}
                    className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm"
                  />
                </div>
`;

const newSearchUI = `
                <div className="flex flex-col md:flex-row gap-2 w-full">
                  <select 
                    value={osStatusFilter}
                    onChange={(e) => setOsStatusFilter(e.target.value)}
                    className="w-full md:w-36 py-2 px-3 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm text-slate-700"
                  >
                    <option value="">Status</option>
                    <option value="PENDING_APPROVAL">Pendente</option>
                    <option value="APPROVED">Aprovada</option>
                    <option value="IN_TRANSIT">Em Trânsito</option>
                    <option value="COMPLETED">Finalizada</option>
                    <option value="CANCELLED">Cancelada</option>
                  </select>
                  <input 
                    type="date"
                    value={osDateFrom}
                    onChange={(e) => setDateFrom(e.target.value)}
                    className="w-full md:w-36 py-2 px-3 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm text-slate-700"
                  />
                  <input 
                    type="date"
                    value={osDateTo}
                    onChange={(e) => setDateTo(e.target.value)}
                    className="w-full md:w-36 py-2 px-3 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm text-slate-700"
                  />
                  <div className="relative w-full md:w-64">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                    <input 
                      type="text" 
                      placeholder="Nº OS ou Placa..."
                      value={osSearch}
                      onChange={(e) => setOsSearch(e.target.value)}
                      className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm"
                    />
                  </div>
                </div>
`;

content = content.replace(/<div className="relative w-full md:w-72">[\s\S]*?<\/div>/, newSearchUI);
content = content.replace(/setDateFrom/g, 'setOsDateFrom').replace(/setDateTo/g, 'setOsDateTo');

fs.writeFileSync(file, content);
console.log('Admin filters patched.');
